...
app.get("/ippatients", (req, res) => {
  const q = "select a.*,b.disease_name from patients a,patient_disease b where a.disease_id=b.id and a.user_types_id ='4' and a.status ='2'";
    db.query(q, (err, data) => {
    if (err) {
      return res.json(err);
    }
...
app.get("/oppatients", (req, res) => {
  const q = "select a.*,b.disease_name from patients a,patient_disease b where a.disease_id=b.id and a.user_types_id ='4' and a.status ='3'";
    db.query(q, (err, data) => {
    if (err) {
      return res.json(err);
    }
